...
public class EntityReader {
    ...
    private static Class<? extends Entity> getClassByCsvFile(final String csvFile) {
        if (csvFile.contains("hive")) {
            return Hive.class;
        } else if (csvFile.contains("colony")) {
            return Colony.class;
        }
        return null;
    }

    private static Entity createEntity(final Class<? extends Entity> entityClass, final List<String> headers,
            final String[] values, Map<Class<? extends Entity>, List<Entity>> entityMap) {
        try {
            ...
            for (int i = 0; i < headers.size(); i++) {
                ...
                if (field != null) {
                    ...
                    if (field.getName().equals("hiveId")) {
                        if (entity instanceof Colony) {
                            int hiveId = Integer.parseInt(value);
                            List<Entity> hives = entityMap.get(Hive.class);
                            Hive hive = null;
                            if (hives != null) {
                                for (Entity e : hives) {
                                    Hive r = (Hive) e;
                            ...
                            field.set(entity, hiveId);
                            Field hiveField = getFieldByName(fields, "hive");
                            if (hiveField != null) {
                                hiveField.setAccessible(true);
                                hiveField.set(entity, hive);
                            }
                        }
                    } else {
    ...
    private static Field getFieldByName(final Field[] fields, final String fileName) {
    ...
    private static Object convertValueToFieldType(final String value, final Class<?> fieldType) {
    ...
    private static void overwriteCSV(final Path path, final List<String> lines, final String headerLine) {
